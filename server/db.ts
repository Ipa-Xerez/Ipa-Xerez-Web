...
// Get featured blog posts for homepage (max 4)
// Priority: featured=1 ordered by featuredOrder ASC, then publishedAt DESC
// Fallback: if less than 4 featured, fill with latest published posts
export async function getFeaturedBlogPosts(limit: number = 4) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  try {
    ...
    const featuredPosts = await db
      ...
      .orderBy(blogPosts.featuredOrder, desc(blogPosts.publishedAt))
      .limit(limit);
    ...
    // Otherwise, fill with latest non-featured published posts
      ...
      recentPosts = await db
        ...
        .where(
          and(
            ...
            sql`${blogPosts.id} NOT IN (${sql.join(featuredIds.map(id => sql`${id}`), sql`, `)})`
          )
        )
        .orderBy(desc(blogPosts.publishedAt))
        .limit(remaining);
